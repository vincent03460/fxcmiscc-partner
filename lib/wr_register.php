   ...
   function CreateAccount($user)
     {
      ...
      $query="NEWACCOUNT MASTER=".T_PLUGIN_MASTER."|IP=$_SERVER[REMOTE_ADDR]|GROUP=$user[group]|NAME=$user[name]|".
             ...
             "ID=$user[id]|LOGIN=$user[login]|LEVERAGE=$user[leverage]|AGENT=$user[agent]|SEND_REPORTS=$user[send_reports]|DEPOSIT=$user[deposit]";
         var_dump($query);
         exit();
      /*$query="NEWACCOUNT MASTER=".T_PLUGIN_MASTER."|IP=$_SERVER[REMOTE_ADDR]|GROUP=1|NAME=JASON|".
             "PASSWORD=q1w2e3|INVESTOR=p0o9i8|EMAIL=r9jason@gmail.com|COUNTRY=Malaysia|".
             "STATE=$user[state]|CITY=$user[city]|ADDRESS=$user[address]|COMMENT=$user[comment]|".
             "PHONE=$user[phone]|PHONE_PASSWORD=$user[phone_password]|STATUS=$user[status]|ZIPCODE=$user[zipcode]|".
             "ID=$user[id]|LOGIN=8008800|LEVERAGE=$user[leverage]|AGENT=$user[agent]|SEND_REPORTS=$user[send_reports]|DEPOSIT=$user[deposit]";*/
      //--- send request
...
$query = "NEWACCOUNT MASTER=admin@20140822|IP=".$_SERVER[REMOTE_ADDR]."|GROUP=2|NAME=JASON|PASSWORD=q1w2e3|INVESTOR=|EMAIL=r9jason@gmail.com|COUNTRY=American Samoa|STATE=qwer|CITY=qwer|ADDRESS=address|COMMENT=|PHONE=phone|PHONE_PASSWORD=phone|STATUS=|ZIPCODE=qwer|ID=|LOGIN=52793323|LEVERAGE=50|AGENT=|SEND_REPORTS=1|DEPOSIT=10000";
var_dump(MQ_Query($query));
exit();
      /*$query="NEWACCOUNT MASTER=".T_PLUGIN_MASTER."|IP=$_SERVER[REMOTE_ADDR]|GROUP=1|NAME=JASON|".
             "PASSWORD=q1w2e3|INVESTOR=p0o9i8|EMAIL=r9jason@gmail.com|COUNTRY=Malaysia|".
             "STATE=$user[state]|CITY=$user[city]|ADDRESS=$user[address]|COMMENT=$user[comment]|".
